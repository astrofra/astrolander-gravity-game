...
//		level_name[1].refresh()
	...
	constructor(_ui)
	{
		...
		local	idx = 0
		local	_max_seasons = g_seasons.len()

		for(idx = 0; idx < g_seasons.len(); idx++)
		{
			//	Fetch season
			local	season = g_seasons["season_" + idx.tostring()]
	...
	function	GoToSeasonSelectionScreen0(event, table)
	{
		...
		PlaySfxUIValidate()
		ButtonFeedback(table.window)
		if (!("current_selected_season" in ProjectGetScriptInstance(g_project).player_data))
			ProjectGetScriptInstance(g_project).player_data.rawset("current_selected_season", 0)
