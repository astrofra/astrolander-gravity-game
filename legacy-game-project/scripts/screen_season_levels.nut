...
class	SeasonLevelsScreenUI	extends BaseUI
{
	...
	//--------------------------------
	{
		local	_bt = current_selected_level

		while(_bt >= 8)
			_bt -= 8

		SpriteSetParent(selector_hilite, level_buttons[_bt])
	}
	...
	//---------------------------------
	{
		...
		for (_y = 0; _y < 2; _y++)
			for(_x = 0; _x < 4; _x++)
			{
				...
				local	_world_level_number = _level_number + (current_world * 8)
				local	_click_handler_button = 0

				if (!IsLevelCompleted(Max(_world_level_number - 1, 0)))
				{
					print("CreateLevelButtonGrid() : Level " + _world_level_number + " is locked. Adding a 'lock' icon.")
					local	_level_lock = UIAddSprite(ui, -1, EngineLoadTexture(g_engine, "ui/lock.png") , 20, 20, 256, 256)
					SpriteSetScale(_level_lock, 0.8, 0.8)
					SpriteSetParent(_level_lock, _level_button)
					_click_handler_button = _level_lock
				}
				else
				{
					local	_level_number_button = LabelWrapper(ui, (_world_level_number + 1).tostring(), 0, 0, 90, 256, 256, Vector(117, 155, 168, 255), g_main_font_name, TextAlignCenter)
					WindowSetParent(_level_number_button[0], _level_button)
					_click_handler_button = _level_number_button[0]
				}

				WindowSetParent(_level_button, grid_parent)
				WindowSetEventHandlerWithContext(_click_handler_button, EventCursorDown, this, SeasonLevelsScreenUI["OnSelectLevel" + _level_number])
				level_buttons.append(_click_handler_button)
			}
	}

	function	OnSelectLevel0(event,table)
	{	OnSelectLevel(0, event, table)	}
	function	OnSelectLevel1(event,table)
	{	OnSelectLevel(1, event, table)	}
	function	OnSelectLevel2(event,table)
	{	OnSelectLevel(2, event, table)	}
	function	OnSelectLevel3(event,table)
	{	OnSelectLevel(3, event, table)	}
	function	OnSelectLevel4(event,table)
	{	OnSelectLevel(4, event, table)	}
	function	OnSelectLevel5(event,table)
	{	OnSelectLevel(5, event, table)	}
	function	OnSelectLevel6(event,table)
	{	OnSelectLevel(6, event, table)	}
	function	OnSelectLevel7(event,table)
	{	OnSelectLevel(7, event, table)	}

	//-------------------------------
	function	OnSelectLevel(_level, event = 0, table = 0)
	//-------------------------------
	{
		local	_clicked_level = _level + (current_world * 8)
		//	Check if the previous level was completed already
		//	If not, notify the user that this level is locked...

		print("OnSelectLevel() : _clicked_level = " + _clicked_level) 

		if (_clicked_level > 0 && !(IsLevelCompleted(_clicked_level - 1)))
		{
			print("OnSelectLevel() Previous level (" + (_clicked_level - 1).tostring() + ") is not completed!")

			if (table != 0)	ButtonFeedback(table.window)
			PlaySfxUIError()
			return
		}

		current_selected_level = _clicked_level
		...
		WindowSetScale(level_thumbnail, 0.6, 0.6)

		if (ObjectIsValid(th_parent))
		{
			local	_parent_pos = SpriteGetPosition(th_parent)
			SpriteSetPosition(level_thumbnail, _parent_pos.x, _parent_pos.y + 42.0)
		}
