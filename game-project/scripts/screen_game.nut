...
//		if ((_dist < Mtr(3.0)) && (player_script.current_speed < Mtrs(0.5)))
	...
	//------------------------------------------
	{
		...
		{
			local	_coin_pos = ItemGetWorldPosition(_coin)
			local	_dist = ItemGetWorldPosition(player).Dist(_coin_pos)

			//	If a bonus was reached
			if (_dist < Mtr(2.5))
			{
				ScenePlayerGetsCoin(scene, _coin)
			}
		}
...
//						ItemActivateHierarchy(_item, false)
	...
	//----------------------------------------------
	{
		print("LevelHandler::ScenePlayerGetsCoin()")
		PlaySfxGotItem()
