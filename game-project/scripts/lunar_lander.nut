...
/*
	...
	{
		print("TriggerGrabber::OnSetup()")
	}

	function	OnItemEnter(trigger_item, item)
	{
		print("TriggerGrabber::OnItemEnter()")
		local	_item_name = ItemGetName(item)
		switch(_item_name)
		{
			case "Coin":
				SceneGetScriptInstance(g_scene).ScenePlayerGetsCoin(g_scene, item)
				break
		}
	}
*/
...
//		SceneGetScriptInstance(g_scene).replay_handler.UpdateItemMotionRecord(g_clock, ItemGetWorldPosition(item), ItemGetRotation(item))
	...
	//---------------------------------
	{
		...
		if (speed_limiter > 0.0 && _speed_limit.Len() < max_speed)
	...
	//------------------------------------------------------
	{
		if (current_speed >= speed_min_damage)
		{
			if (!shield_enabled)
			{
				...
				local	k_damage = Abs(col_normal.Dot(current_velocity.Normalize())) //Max(-col_normal.Dot(_cur_vel_n), 0.0)
